...
if (isset($_GET['jbm'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo '<td>' . $row['loc'] . '</td>';
            echo '<td><a class="btn btn-sm btn-primary" href="deb_des.php?t=' . $row['id'] . '"><i class="far fa-plus"></i> Debit (بردگی</a></td>';
            echo '<td><a class="btn btn-sm btn-success" href="cre_des.php?id=' . $row['id'] . '"><i class="far fa-plus"></i>  Credit (رسیدگی</a></td>';
            echo '</tr>';
        }
        echo '</tbody></table>';
    } else {
