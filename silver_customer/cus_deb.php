...
<body class="bg-light">
    ...
    if (isset($_POST['insert'])) {
        ...
        if ($Paidc == "Cash") {
            ...
            $crea = "INSERT INTO `account`(`cus_id`, `ref`, `amount`, `amo_cre`, `gold`, `role`, `state`, `type`, `com`) VALUES ('$id','$ref','$price','0','0','Silver Customer','$Paid','Debit','$com')";
            mysqli_query($conn, $crea);

            $debsh = "INSERT INTO `storage`(`st_cus`, `st_price`, `st_gold`, `type`, `method`, `name`) VALUES ('$id','$price','0','Cash','Debit','money')";
            mysqli_query($conn, $debsh);

            $stah = "INSERT INTO `statement`(`se_cus`, `ref`, `se_tgolg`, `se_tpurity`, `se_tbill`, `se_type`, `se_method`, `dis`) VALUES ('$id','$ref','0','0','$price','Debit','$Paid', 'PAID <br> ($price$ With $COM CASH $com$)')";
            mysqli_query($conn, $stah);

            $msg = '
            <div class="alert alert-info alert-dismissible fade show" role="alert">
                <i class="fa fa-exclamation-circle me-2"></i>Silver Customer Debit Money Added!
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
        ';
        } else if ($Paidc == "in Account") {

            $cre = "INSERT INTO `account`(`cus_id`, `ref`, `amount`, `amo_cre`, `gold`, `role`, `state`, `type`, `com`) VALUES ('$id','$ref','$totalcc','0','0','Silver Customer','$Paid','Debit','$com')";
            $res = mysqli_query($conn, $cre);

            $cre = "INSERT INTO `debit`(`dep_cus`, `ref`, `dep_tgold`, `dep_tbill`, `dep_tpurity`, `sate`, `com`, `typec`) VALUES ('$id','$ref','0','$price','0','0','Debit Commission','$com','$Paidc')";
            mysqli_query($conn, $cre);

            $debs = "INSERT INTO `storage`(`st_cus`, `st_price`, `st_gold`, `type`, `method`, `name`) VALUES ('$id','$totalcc','0','Cash','Debit','money')";
            mysqli_query($conn, $debs);

            $sta = "INSERT INTO `statement`(`se_cus`, `ref`, `se_tgolg`, `se_tpurity`, `se_tbill`, `se_type`, `se_method`, `dis`) VALUES ('$id','$ref','0','0','$totalcc','Debit','$Paid', 'PAID <br> ($price$ With $COM In Account $com$)')";
            ...
            <div class="alert alert-danger alert-dismissible fade show" role="alert">
                <i class="fa fa-exclamation-circle me-2"></i>Silver Customer Credit Money Not Added!
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
        ';
                        ...
                        <form action="" method="post" enctype="multipart/form-data">
                            <div class="card">
                                <div class="card-header text-center bg-primary ">
                                    <h3 class="text-white mt-2">Add Silver Customer Debit Money</h3>
                                </div>
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-4 my-2">
                                            <div class="form-group">
                                                <label for="Paidc" class="text-primary">Piad Method Commission</label>
                                                <select id="Paidc" name="Paidc" class="form-select">
                                                    <option value="------">------</option>
                                                    <option value="Cash">Cash</option>
                                                    <option value="in Account">in Account</option>
                                                </select>
