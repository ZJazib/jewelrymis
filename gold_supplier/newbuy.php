...
<body class="bg-light">
    ...
    if (isset($_POST['insertn'])) {
        ...
        $debc = "INSERT INTO `credit`(`ref`, `cre_cus`, `cre_tgold`, `cre_tbill`, `cre_tpurity`, `ujorat`, `com`, `sate`, `fix`, `role`, `typec`) VALUES ('$ref','$id','$goldg','$mc','$purity','0','0','Credit','not','Supllier','$Paid')";
        mysqli_query($conn, $debc);

        $crec = "INSERT INTO `account`(`cus_id`, `ref`, `amount`, `amo_cre`, `gold`, `role`, `state`, `type`, `com`) VALUES ('$id','$ref','0','$mc','$purity','Supplier','$Paid','Credit','0')";
        $resc = mysqli_query($conn, $crec);

        $debsc = "INSERT INTO `storage`(`st_cus`, `st_price`, `st_gold`, `type`, `method`, `name`) VALUES ('$id','0','$purity','$Paid','Credit','gold')";
        mysqli_query($conn, $debsc);

        $stac = "INSERT INTO `statement`(`se_cus`,`ref`, `se_tgolg`, `se_tpurity`, `se_tbill`, `se_type`, `se_method`, `dis`) VALUES ('$id','$ref','$goldg','$purity','$mc','Credit','$Paid', 'JEWELLERY ISSUED <br> (GROSS WT- $goldg GMS) ')";
            ...
            <div class="container py-5 ">
                <div class="row">
                    ...
                    <div class="col-md-4 bg-dark rounded m-3">
                        ...
                        <?php
                        $sqlcc = "SELECT SUM(gold) as totalkgc FROM `account` where `type`='Credit' and cus_id=" . $_GET['id'];
                        $resc = mysqli_query($conn, $sqlcc);
                        $cusc = mysqli_fetch_assoc($resc);

                        $sqlg = "SELECT SUM(gold) as totalkg FROM `account` where `type`='Debit' and cus_id=" . $_GET['id'];
                        $resg = mysqli_query($conn, $sqlg);
                        $cusg = mysqli_fetch_assoc($resg);

                        $totalg = floatval($cusg['totalkg']);
                        $totalkg = floatval($cusc['totalkgc']);
                        ?>
                        <h5 class="text-white px-4">Debit Gold: <span> <?= number_format($totalg, 3) ?> </span> </h5>
                        <h5 class="text-white px-4">Credit Gold: <span> <?= number_format($totalkg, 3) ?></span></h5>
                        <h5 class="text-white ">
                            <?php

                            if ($totalg > $totalkg) {
                                echo "<span class='text-white mx-4'>Total Debit: ", number_format($totalg - $totalkg, 3), "</span>";
                                # code...
                            } else {
                                echo "<span class='px-4 text-white p-2 text-center w-100'>Total Credit: ", number_format($totalkg - $totalg, 3), "</span>";
                                # code...
                            }
                            ?></h5>
                    </div>
