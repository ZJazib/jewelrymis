...
use Dompdf\Options;
  ...
  if(isset($_POST['insert'])){
    ...
    @$totalpure = $_POST['totalpure'];
        ...
        $deb = "INSERT INTO `debit`(`dep_cus`, `ref`, `dep_tgold`, `dep_tbill`, `dep_tpurity`, `sate`, `com`, `typec`) VALUES ('$id','$ref','$totalg','$bill','$totalpure','Credit','0','$Paid')";
        mysqli_query($conn, $deb);

        $amo = "INSERT INTO `account`(`cus_id`, `ref`, `amount`, `amo_cre`, `gold`, `role`, `state`, `type`, `com`) VALUES ('$id','$ref','$bill','0','$totalpure','Customer','$Paid','Credit','0')";
        mysqli_query($conn, $amo);

        $debs = "INSERT INTO `storage`(`st_cus`, `st_price`, `st_gold`, `type`, `method`, `name`) VALUES ('$id','0','$totalpure','$Paid','Credit','gold')";
        mysqli_query($conn, $debs);

        $sta = "INSERT INTO `statement`(`se_cus`,`ref`, `se_tgolg`, `se_tpurity`, `se_tbill`, `se_type`, `se_method`, `dis`) VALUES ('$id','$ref','$totalg','$totalpure','$bill','Credit','$Paid', 'JEWELLERY SOLD REVISED <br> (GROSS WT - $totalg GMS)')";
        mysqli_query($conn, $sta);


    $goldCount = count($_POST['dis']);

    // Initialize the HTML content for the invoice
    ...
    $invoiceHtml .= '<tr>';
    $invoiceHtml .= '<td colspan="8">' . number_format($billFloat) . ' $ CREDITED</td>';
